...
Flow of contents
1. Creating a Series: from a list, dictionary, ndarray or scalar value; with indexing, MultiLevel Indexed Series
2. Accessing elements: s.iloc[], dictionary style s[], s.get() method
3. Copying a Series: s.copy()
4. Updating elements: s.iloc[]=, dictionary style s[]=, s.update()
5. Add new elements: s[]=, pd.concat()
6. Deleting elements: s.drop(), s.pop()
'''
